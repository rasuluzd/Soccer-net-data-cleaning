              ...
              {result.clips?.map((clip, i) => (
                <div key={i} className="bg-black/30 backdrop-blur-xl border border-white/8 rounded-2xl overflow-hidden">
                  ...
                  <div className="px-3 pb-3">
                    {/* The video file is the full match, so seek into it using
                        clip.start. Part 2 needs `part2_offset_sec` added — that
                        offset is per-match metadata in registry.json. */}
                    {(() => {
                      const offset =
                        clip.part === 2 ? clip.video?.part2_offset_sec ?? 0 : 0;
                      const videoStart = clip.start + offset;
                      const videoEnd = clip.end + offset;
                      const src =
                        clip.video?.parts?.[String(clip.part)] ||
                        clip.video?.parts?.["1"] ||
                        "";
                      return (
                        <VideoPlayer
